...
import { RideHistoryProps, updateTripProgress } from '@/types/vehicle';
...
const passengers = [
  ...
  },
  {
    ...
    coordinate: { latitude: 6.526, longitude: 3.38 },
  },
  {
    id: '3',
    name: 'Ngozi',
    image: { uri: 'https://randomuser.me/api/portraits/women/21.jpg' },
    coordinate: { latitude: 6.523, longitude: 3.378 },
  },
  {
    id: '4',
    name: 'Femi',
    image: { uri: 'https://randomuser.me/api/portraits/men/55.jpg' },
    coordinate: { latitude: 6.522, longitude: 3.377 },
  },
];
...
export default function TripMonitoring() {
  ...
  //   rideid: trip?.id ?? '',
  // });
  // const tripService = new TripWs(currentLocation);
  // await tripService.connect(userid, rideid);

  // tripService.on('tripStarted', (data) => {
  //   console.log('Trip started!', data);
  // });

  // tripService.sendStartTrip();
  ...
  const handleSelectPassenger = (passenger: (typeof passengers)[0]) => {
  ...
  const initializeTripMonitoring = async () => {
    try {
      ...
      // initiaize websocket connection
      // connect();
  ...
  return (
    <>
      {/* <Modal visible={modalVisible} animationType="slide">
        <View className="flex-1 bg-black/50 justify-center items-center px-6  absolute top-0 left-0 right-0 bottom-0">
          <View className="bg-white rounded-2xl p-6 w-full h-full flex-1 max-h-[60%]">
            <ScrollView
            ...
            >
              ...
              <Text className="text-lg font-semibold mb-2">
                Actions for {selectedPassenger?.name}
              </Text>
              <View className="flex-row gap-2">
                <TouchableOpacity
                  className="bg-red-500 py-3 rounded-xl mb-3"
                  onPress={() => {
                    setModalVisible(false);
                    alert(`Cancelled trip for ${selectedPassenger?.name}`);
                  }}
                >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    setModalVisible(false);
                    alert(`Ended trip for ${selectedPassenger?.name}`);
                  }}
                >
                  <Text className="text-white text-center font-medium">
                    End Trip
        ...
        >
          ...
          {passengers.map((p) => (
            <Marker key={p.id} coordinate={p.coordinate}>
              <BlinkingBeacon imageSrc={p.image} name={p.name} />
            </Marker>
          ))}
          ...
          <Marker
            ...
            description={trip.pickup_location?.address || ''}
            pinColor="green"
          />
            ...
            <View className="ml-2 flex-1">
              <View className="">
                ...
                <Text className="text-xs text-gray-500">
                  {trip.driver?.driver?.model} • 
                  {trip.driver?.driver?.interior_color} • 
                  {trip.driver?.driver?.plate_number}
                </Text>
              </View>
              <View className="flex-row justify-between items-center ">
                <View className="flex-row gap-2">
                  ...
                  <View className="flex-row">
                    {trip?.driverStatistic?.starDistribution &&
                      trip.driverStatistic.starDistribution.map(
                        (item, _index) => (
                          <Feather
                            key={_index}
                            name="star"
      ...
      >
        ...
        >
          ...
          <View className="flex-row justify-between gap-2 items-center mb-14">
            {(currentUser?.accountType ==="passenger" && user.accountType ==="driver") || (currentUser?.id === user.id && user.accountType==="driver")  ? (<>
            <Pressable
              onPress={handleCall}
              className="flex-1 justify-center items-center py-1 border border-primary-900 rounded-md flex-row gap-1"
            >
